...
from eegnb.stimuli import LANGUAGES

__title__ = "Foreign Languages"
...
def present(duration=100, eeg=None, save_fn=None):
    ...
    markernames = [1, 2, 3]
    n_trials = int(duration / (soa + iti + jitter))
    ...
    english = list(map(load_image, glob(os.path.join(LANGUAGES, "english", "*.png"))))
    latin = list(map(load_image, glob(os.path.join(LANGUAGES, "latin", "*.png"))))
    nonlatin = list(map(load_image, glob(os.path.join(LANGUAGES, "nonlatin", "*.png"))))

    stim = [english, latin, nonlatin]
    ...
    if eeg:
        if save_fn is None:  # If no save_fn passed, generate a new unnamed save file
            save_fn = generate_save_fn(eeg.device_name, "foreign_language", "unnamed")
            print(
    ...
    for ii, trial in trials.iterrows():
        ...
        image = choice(english if label == 2  else (latin if label == 1 else nonlatin))
        image.draw()
