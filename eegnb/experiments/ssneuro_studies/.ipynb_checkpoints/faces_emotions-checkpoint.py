...
from eegnb.stimuli import FACES_EMOTIONS
...
def present(subject, session, eeg=None, save_fn=None, isi = 0.5, jitter = 0, duration = 120):
    ...
    # Setup stimuli
    happy_paths = list(glob(os.path.join(FACES_EMOTIONS, "happy", "*.jpg")))
    ...
    if eeg:
        if save_fn is None:  # If no save_fn passed, generate a new unnamed save file
            save_fn = generate_save_fn(eeg.device_name, "faces_emotions", "unnamed")
            print(
    ...
    for iTrial in range(0, n_trials):
        ...
        print(
            (
                "Running Trial %d: Trial Type %d, ISI = %.1f"
                % (iTrial, TrialType, params["ISI"])
            )
        )
    ...
    # display results
    message1.setText("That's the end! Press space to continue.")
    message1.draw()
    win.flip()
