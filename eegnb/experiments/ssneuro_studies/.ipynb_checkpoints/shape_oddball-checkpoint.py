...
def present(subject, session, eeg=None, save_fn=None, goStimProb = 0.34, isi = 0.5, jitter = 0, duration=120):
    ...
    markernames = [1, 2] # 1 = standard, 2 = target
    ...
    params = {
        ...
        "respKeys": ["space"],
        "goStim": "triangle",  # shape signaling respond
    ...
    tNextFlip = [0.0]
    stims = {"square": [], "star": [], "circle": [], "triangle": []}
    stims["square"] = visual.Rect(
        win,
        lineColor="#000000",
        lineWidth=3.0,
        size = (fCS_rt2, fCS_rt2),
        pos = (0, 0),
        units="pix",
    )
    stims["star"] = visual.ShapeStim(
        win,
        lineColor="#000000",
        lineWidth=3.0,
        #vertices=(
        ...
        pos = (0, 0),
        vertices = [
            (0.0, 1.0), (0.2352, 0.3236), (0.9511, 0.3090), (0.3804, -0.1236),
            (0.5878, -0.8090), (0.0, -0.4), (-0.5878, -0.8090), (-0.3804, -0.1236),
            (-0.9511, 0.3090), (-0.2352, 0.3236)
        ],
        size = fCS_rt2,
        units='pix',
        name="star",
    ...
    def show_instructions():
        message1.setText("Welcome to the experiment! Press the space bar each time you see a triangle. Do not press at any other point.")
    ...
    if eeg:
        if save_fn is None:  # If no save_fn passed, generate a new unnamed save file
            save_fn = generate_save_fn(eeg.device_name, "shape_oddball", "unnamed")
            print(
    ...
    for iTrial in range(0, n_trials):
        ...
        # Decide Trial Params
        isGoTrial = random.random() < goStimProb # approx. 1/3 probability of triangle
        isStar = random.random() < 0.05 # 5% probability of getting a star
        isOrange = random.random() < 0.05 # 5% probability of orange stimulus
        
        # set random color of options
        ...
        if isGoTrial:
            label = 0
            image = stims["triangle"]
            
            # set color
            if isOrange:
                image.fillColor = 'orange'
            elif ~isOrange:
                image.fillColor = choice(colorArray)
            image.draw()
            win.logOnFlip(
            ...
            if isStar:
                ...
                image_choice = choice(shapeArray)
                image = stims[image_choice]
            # set color
            if isOrange:
                image.fillColor = 'orange'
            elif ~isOrange:
        ...
        image_text = image.name
        image_color = image.fillColor
        tempArray = [iTrial, image_text, image_color, isGoTrial, respKey, t]
        responses.append(tempArray)
