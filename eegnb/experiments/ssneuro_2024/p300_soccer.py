...
from eegnb.stimuli import SOCCER
...
def present(duration=120, eeg=None, save_fn=None):
    ...
    mywin.flip()
    mywin.flip()
    targets = list(map(load_image, glob(os.path.join(SOCCER, "target", "*.jpg"))))
    nontargets = list(map(load_image, glob(os.path.join(SOCCER, "nontarget", "*.jpg"))))
...
def show_instructions(duration, ver):
    ...
    Stay still, focus on the centre of the screen, and try not to blink. Try to count the number of times you see the soccer player Neymar.
